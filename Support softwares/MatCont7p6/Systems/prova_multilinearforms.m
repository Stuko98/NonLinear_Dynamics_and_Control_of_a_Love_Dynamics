function F = prova_multilinearforms
F.A = @A;
F.J1 = @J1;
F.B = @B;
F.A1 = @A1;
...
t4 = exp(t2);
t5 = exp(t3);
A = reshape([-1.0,k_.*t4+k_.*t2.*t4,k_.*t5+k_.*t3.*t5,-1.0],[2,2]);
end
...
t4 = exp(t2);
t5 = exp(t3);
B = [-v12.*v22.*(k_.*t5.*2.0+k_.*t3.*t5);-v11.*v21.*(k_.*t4.*2.0+k_.*t2.*t4)];
end
...
t4 = exp(t2);
t5 = exp(t3);
C = [v12.*v22.*v32.*(k_.*t5.*3.0+k_.*t3.*t5);v11.*v21.*v31.*(k_.*t4.*3.0+k_.*t2.*t4)];
end

...
%% PER LA VERSIONE 2025b sono state modificate alcune righe(DA ME)! 
...
%% MODIFICA CON CHATGPT per la versione 2025b
  % Menus

  hhsetup = get(0,'ShowHiddenHandles');
  set(0,'ShowHiddenHandles','on');

  mefile = findobj(ppset,'Type','uimenu','Label','&File');
  if isempty(mefile) || ~ishandle(mefile)
      mefile = uimenu(ppset,'Label','&File');
  end

  meedit = findobj(ppset,'Type','uimenu','Label','&Edit');
  if isempty(meedit) || ~ishandle(meedit)
      meedit = uimenu(ppset,'Label','&Edit');
  end
%%

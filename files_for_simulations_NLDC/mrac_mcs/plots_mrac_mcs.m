...
%exportgraphics(fig, 'x_P2_phase_plane_mrac_mcs.pdf');
...
plot(out.tout,out.logsout.get('xm').Values.Data(:,1),'LineWidth',2)    % xm1
plot(out.tout,yd*ones(size(out.tout)),'LineStyle','--','LineWidth',2) % yd
hold off
ylabel('$y(t)$, $x_{m1}(t)$, $y_d$')
xlabel('$t$ [s]')
ylim padded
grid on
box on
legend('$y(t) = x_1(t)$','$x_{m1}(t)$','$y_d$','FontSize',12,'Location','southeast'); 
nexttile
...
grid on
box on
%exportgraphics(fig, 'y_x2_mrac_mcs.pdf');
...
grid on
box on 
%exportgraphics(fig, 'u_mrac_mcs.pdf');


% K(t), kr(t)
fig = figure;
plot(out.tout, out.logsout.get('K(t), kr(t)').Values.Data, 'LineWidth', 2); % K(t), kr(t)
ylabel('$K(t), k_r(t)$')
xlabel('$t$ [s]')
ylim padded
grid on
box on 
legend('$K_1(t)$','$K_2(t)$','$k_r(t)$','FontSize',12,'Location','southeast'); 
%exportgraphics(fig, 'K_kr_mrac_mcs.pdf');


% e
fig = figure;
plot(out.tout,out.logsout.get('e_real').Values.Data, 'LineWidth', 2); % e_real = yd - y
ylabel('$e(t)$')
xlabel('$t$ [s]')
ylim padded
grid on
box on 
%exportgraphics(fig, 'e_real_mrac_mcs.pdf');

...
%% plots
% x vs switching manifold vs P2 in the phase plane
fig = figure;
hold on 
plot(out.logsout.get('x').Values.Data(:,1),out.logsout.get('x').Values.Data(:,2),'LineWidth',2)                  % (x1,x2)
plot(out.logsout.get('x1_sm').Values.Data,out.logsout.get('x2_sm').Values.Data,'LineWidth',1,'LineStyle','--')   % switching manifold
plot(x1d,x2d,'LineWidth',2,'Marker','*')                                                                         % (x1,x2d) 
hold off
axis padded
grid on
box on
ylabel('$x_2(t)$')
xlabel('$x_1(t)$')
legend('$x(t)$','$\Sigma$','$P_2$','FontSize',12,'Location','southeast'); 
% Inset (finestrella di zoom) su sigma(x)
% Specifica la posizione dell'inset [x, y, width, height] in unità normalizzate (relative alla figure)
inset_ax = axes('Position', [0.65 0.4 0.2 0.2]);  % adatta se serve
box on
% Dati da plottare nello zoom
hold on 
plot(out.logsout.get('x').Values.Data(:,1),out.logsout.get('x').Values.Data(:,2),'LineWidth',2)                 
plot(out.logsout.get('x1_sm').Values.Data,out.logsout.get('x2_sm').Values.Data,'LineWidth',1,'LineStyle','--')  
plot(x1d,x2d,'LineWidth',2,'Marker','*')                                                                       
hold off
xlim([2.597 2.602]);       % <-- scegli tu la porzione di tempo
ylim([2.597 2.602]);       % <-- scegli tu la porzione verticale
set(inset_ax, 'FontSize', 8);
grid on
%exportgraphics(fig, 'x_sigma_P2_phase_plane_smc.pdf');
...
grid on
box on
%exportgraphics(fig, 'y_x2_smc.pdf');
...
grid on
box on 
%exportgraphics(fig, 'u_smc.pdf');


% sigma(x)
fig = figure;
plot(out.tout, out.logsout.get('sigma').Values.Data, 'LineWidth', 2); % sigma(x)
ylabel('$\sigma(x(t))$')
xlabel('$t$ [s]')
ylim padded
grid on
...
% Inset (finestrella di zoom) su sigma(x)
% Specifica la posizione dell'inset [x, y, width, height] in unità normalizzate (relative alla figure)
inset_ax = axes('Position', [0.6 0.6 0.2 0.2]);  % adatta se serve
box on
% Dati da plottare nello zoom
plot(out.tout, out.logsout.get('sigma').Values.Data, 'LineWidth', 2); % sigma(x)
xlim([4 4.01]);       % <-- scegli tu la porzione di tempo
ylim([-2e-4 2e-4]);   % <-- scegli tu la porzione verticale
set(inset_ax, 'FontSize', 8);
grid on
%exportgraphics(fig, 'sigma.pdf');
...
x_ = out.logsout.get('x').Values.Data;
y_ = x_(:,1);              % output y = x1

r_ = yd * ones(size(t_));  % constant reference

u_ = out.logsout.get('u').Values.Data(:);

e_ = r_ - y_;

%% User-defined options
% tempo di assestamento al (100-epsilon)%
settlingTolerance = 0.05;     % == 5% of the final value of y == settling band for settling time ts. MODIFICA QUI
finalSamples = min(50, length(t_));

%% Final values

r_final = mean(r_(end-finalSamples+1:end));
y_final = mean(y_(end-finalSamples+1:end));
u_final = mean(u_(end-finalSamples+1:end));
e_final = mean(e_(end-finalSamples+1:end));
...
maximum_absolute_error = max(abs(e_));
...
if rf >= y0
    idx10 = find(y_ >= y10, 1, 'first');
    idx90 = find(y_ >= y90, 1, 'first');
else
    idx10 = find(y_ <= y10, 1, 'first');
    idx90 = find(y_ <= y90, 1, 'first');
end
...
inside_settling_band = abs(y_ - rf) <= settling_band;
...
maximum_output_value = max(y_);

% Maximum overshoot percentage and peak time
if rf >= y0
    [peak_value, idx_peak] = max(y_);
...
peak_time = t_(idx_peak);

%% ============================================================
%  Control effort specifications
% ============================================================

maximum_control_effort = max(abs(u_));

control_minimum_value = min(u_);

control_maximum_value = max(u_);
